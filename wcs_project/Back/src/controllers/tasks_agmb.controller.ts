...
import { AgmbStoreService } from '../services/tasks_agmb.service';

dotenv.config();

const agmbStoreService = new AgmbStoreService();
...
export const changeToProcessingBatch = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const response = await agmbStoreService.changeToProcessingBatch(
            dto,
            reqUsername
        );
...
export const handleOrderItemAgmb = async (req: Request, res: Response) => {
    const operation = 'TasksTMController.handleOrderItemAgmb';
    ...
    try {
        const response = await agmbStoreService.handleOrderItemAgmb(order_id, actual_qty, reqUsername);
        return ResponseUtils.handleResponse(res, response);
    } catch (error: any) {
        console.error(`Error during ${operation}:`, error);
...
export const handleErrorOrderItemAgmb = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const response = await agmbStoreService.handleErrorOrderItemAgmb(
            Number(event_id),
            items,
            reqUsername
        );
...
export const handleManualOrder = async (req: Request, res: Response) => {
    ...
    try {
        const response = await agmbStoreService.handleManualOrder(items, reqUsername);
        return ResponseUtils.handleResponse(res, response);
    } catch (error: any) {
        console.error(`Error during ${operation}:`, error);
        return ResponseUtils.handleErrorCreate(
