...
/**
 * task_mrs_detail: รายการย่อยของ task สำหรับฝั่ง MRS
 * - บันทึกว่า MRS เครื่องไหน เปิด/ปิด aisle ไหน ใช้ค่าควบคุมอะไร ผลลัพธ์เป็นอย่างไร
 * - ใช้เป็นทั้ง "detail" และ "log ต่อรอบงาน" ได้
 */
@Entity({ name: 'task_mrs_detail' })
export class TaskMrsDetail {
    /** รหัสรายละเอียด (PK) */
    @PrimaryGeneratedColumn({ type: 'bigint', unsigned: true, comment: 'Primary key of task_mrs_detail' })
    id!: string;
