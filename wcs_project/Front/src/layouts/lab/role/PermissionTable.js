...
const PermissionTable = ({
  ...
  handleMenuChange,
  handleActionChange,
  handleSelectAllMenusChange,
  selectAllMenus,
}) => {
  const renderMenuRows = (menus) =>
    menus.map((menu) => {
      const menuName = lang.msg(menu.menu_name);

      return (
        <React.Fragment key={`menu-${menu.menu_id}`}>
          {/* ระดับ 1 */}
          ...
          {Array.isArray(menu.children) &&
            menu.children.map((submenu) => (
              <React.Fragment key={`submenu-${submenu.menu_id}`}>
                {!submenu.isHeader && (
                  <TableRow>
                    <TableCell>
                      <Checkbox
                    ...
                    <TableCell>
                      {(submenu.menu_actions ?? []).map((action) => (
                        <div key={`act-${submenu.menu_id}-${action.action_code}`}>
                          <Checkbox
                            checked={!!action.checked}
                            onChange={() =>
                              handleActionChange(submenu.menu_id, action.action_code)
                            }
                          />
                          {lang.msg(action.action_name)}
                        </div>
                      ))}
                    </TableCell>
                  </TableRow>
                ...
                {submenu.isHeader && (
                  <React.Fragment key={`subheader-${submenu.menu_id}`}>
                    <TableRow style={{ backgroundColor: "#f0f0f0" }}>
                      <TableCell />
                      <TableCell sx={{ pl: 4 }}>
                    ...
                    {(submenu.children ?? []).map((grandchild) => (
                      <TableRow key={`grand-${grandchild.menu_id}`}>
                        <TableCell>
                          <Checkbox
                        ...
                        <TableCell>
                          {(grandchild.menu_actions ?? []).map((action) => (
                            <div key={`act-${grandchild.menu_id}-${action.action_code}`}>
                              <Checkbox
                                checked={!!action.checked}
                                onChange={() =>
                                  handleActionChange(grandchild.menu_id, action.action_code)
                                }
                              />
                              {lang.msg(action.action_name)}
                            </div>
