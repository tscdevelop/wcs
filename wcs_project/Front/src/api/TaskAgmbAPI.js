...
class ExecutionAgmbAPI {
  ...
  static async createTaskAgmb(payload) {
    try {
      const token = GlobalVar.getToken();
      const endpoint = "/api/execution-agmb/change-to-processing";
  ...
  static async handleOrderItemAgmb(order_id, actual_qty) {
      try {
          const token = GlobalVar.getToken();
          const endpoint = `/api/execution-agmb/handle-order-item-agmb/${order_id}/${actual_qty}`;
  ...
  static async handleErrorOrderItemAgmb(event_id, items) {
    try {
      const token = GlobalVar.getToken();
      const endpoint = "/api/execution-agmb/handle-error-order-item-agmb";
      ...
      console.error("Error in handleErrorOrderItemAgmb:", error);
      throw error;
    }
  }
  ...
  static async handleManualOrder(items) {
    try {
        ...
        const token = GlobalVar.getToken();
        const endpoint = "/api/execution-agmb/handle-manual-order-item-agmb";
...
export default ExecutionAgmbAPI;
